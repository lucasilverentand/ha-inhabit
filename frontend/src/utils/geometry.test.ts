...
describe("angleBetween", () => {
  it("should calculate angle in degrees", () => {
    ...
    expect(angleBetween({ x: 0, y: 0 }, { x: 0, y: 1 })).to.equal(90);
    // Horizontal left
    expect(angleBetween({ x: 0, y: 0 }, { x: -1, y: 0 })).to.be.closeTo(
      180,
      0.01,
    );
  });
...
describe("pointInPolygon", () => {
  ...
  it("should return false for degenerate polygon", () => {
    ...
    expect(pointInPolygon({ x: 0, y: 0 }, { vertices: [{ x: 0, y: 0 }] })).to.be
      .false;
  });
});
...
describe("pointNearLine", () => {
  it("should detect point near line segment", () => {
    expect(pointNearLine({ x: 5, y: 1 }, { x: 0, y: 0 }, { x: 10, y: 0 }, 2)).to
      .be.true;
    expect(pointNearLine({ x: 5, y: 5 }, { x: 0, y: 0 }, { x: 10, y: 0 }, 2)).to
      .be.false;
  });

  it("should handle point at endpoint", () => {
    expect(pointNearLine({ x: 0, y: 0 }, { x: 0, y: 0 }, { x: 10, y: 0 }, 1)).to
      .be.true;
    expect(pointNearLine({ x: 10, y: 0 }, { x: 0, y: 0 }, { x: 10, y: 0 }, 1))
      .to.be.true;
  });

  it("should handle point beyond segment", () => {
    // Point is on the line extended, but beyond the segment
    expect(pointNearLine({ x: 15, y: 0 }, { x: 0, y: 0 }, { x: 10, y: 0 }, 1))
      .to.be.false;
  });
});
